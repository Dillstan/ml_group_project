...
def rank_media(year_range, tmdb_ids, num_places):
    ...
    median_date = (year_range[0] + year_range[1]) / 2
    if (median_date % 2 == 0):
        median_date = date(median_date, 1, 1)
    else:
        median_date = date(int(median_date), 7, 2) # middle day of the year

    # for each frequency, append movies in order of them being closest to the median date

    for freq, movie_set in grouped_by_freq.items():
        freq_rankings = []
    ...
    return [movie[1] for movie in rankings[:5]]
